        ...
        @if(sizeof($lists) > 0)
            @foreach($lists as $key => $list)
                <tr>
                    ...
                    <td class="center"> {{ $list['info']['sku'].'-PID-'.$list['info']['working_id'] }}</td>
...
<td class="center">
    <a class="waves-effect waves-grey btn white modal-trigger m-b-xs amber lighten-3"
       href="#modal{{ $list['info']['working_id'] }}">Check</a>
    <div id="modal{{ $list['info']['working_id'] }}" class="modal"
         style="z-index: 1003; display: none; opacity: 0; transform: scaleX(0.7); top: 250.516304347826px;">
        <div class="col s12">
            <div class="page-title">
                Đang Check Job: {{ $list['info']['sku'].'-PID-'.$list['info']['working_id'] }}
            </div>
        </div>
                    ...
                    <div class="card-content">
                                                    ...
                                                    <div class="card-content center">
                                                        ...
                                                        <div class="row">
                                                            <span class="waves-effect waves-light btn red lighten-2 js-redo-new-submit"
                                                                  order-id="{{ $order['id'] }}" working-id="{{ $list['info']['working_id'] }}"
                        ...
                        <div class="row">
                            <a title="Yêu cầu Designer làm lại design đã thiết kế. Cần đưa thêm lý do để Designer làm lại."
                                class="waves-effect waves-light btn red m-b-xs js-btn-redo" order_id="{{ $list['info']['working_id'] }}">
                                <i class="material-icons left">thumb_down</i>Làm Lại Cả Job</a>
                            <div class="js-redo-form-{{ $list['info']['working_id'] }}" style="display: none;">
                                <form action="{{ url('redo-designer') }}" method="post" class="col s12">
                                    {{ csrf_field() }}
                                    <div class="row">
                                        <div class="input-field col s10">
                                            <i class="material-icons prefix">mode_edit</i>
                                            <input type="text" style="display: none;" name="order_id" value="{{ $list['info']['working_id'] }}"/>
                        ...
                        <div class="row">
                            ...
                            <div class="col s6">
                                <div class="card white">
                                    ...
                                    <div class="row">
                                        <div class="card-content center">
                                            @if(array_key_exists($list['info']['working_id'], $images))
                                                @foreach($images[$list['info']['working_id']] as $img)
                                                ...
                                                @endforeach
                                                <?php unset($images[$list['info']['working_id']]); ?>
                                            @endif
                                        </div>
                                    </div>
    ...
    @if ($list['info']['status'] == env('STATUS_WORKING_CUSTOMER'))
        <a working_id="{{ $list['info']['working_id'] }}" design_id="{{ $list['info']['design_id'] }}"
