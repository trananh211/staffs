    ...
    public function fulfillment()
    {
        ...
        if (sizeof($templates) == 0) {
            ...
            if (sizeof($lists) > 0) {
                ...
                $this->startFulfillExcel($list_orders);
    ...
    public function uploadFileDriver()
    {
        ...
        if (sizeof($templates) == 0) {
            ...
            if (sizeof($lists) > 0) {
                ...
                //Kết thúc Chuẩn bị dữ liệu để tạo file excel
                $this->startUploadFileDriver($list_files_upload, $lst_suppliers);
            } else {
                logfile('-- Đã hết file custom để Upload fulfill. Chuyển sang fulfill file Product Auto');
                $this->uploadFileProductAuto();
            }
        }
        die();
    }

    private function uploadFileProductAuto()
    {
        echo '<br>chua lam<br>';
    }

    private function startUploadFileDriver($lists , $lst_supplier_id)
    {
        $today = date('m_d_Y');
        $google_folder = array();
